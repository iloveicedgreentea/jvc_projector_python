...
class Commands(Enum):
    ...
    # NX models and up
    pm_frame_adapt = b"\x21\x89\x01\x50\x4D\x50\x4D\x30\x42\x0A"
    ...
    pm_pana_pq = b"\x21\x89\x01\x50\x4D\x50\x4D\x31\x36\x0A"
    ...
    #menu controls
    # TODO: simplify all these to just use a base value + param
    ...
    laser_dim_off = b"\x21\x89\x01\x50\x4D\x44\x43\x30\x0a"
    laser_dim_auto1 = b"\x21\x89\x01\x50\x4D\x44\x43\x31\x0a"
    laser_dim_auto2 = b"\x21\x89\x01\x50\x4D\x44\x43\x32\x0a"
    ...
    # todo: eshift, info button, laser dimming
...
class ACKs(Enum):
    ...
    menu_ack = b"\x06\x89\x01\x52\x43\x0A"


class JVCProjector:
    """JVC Projector Control"""

    def __init__(self, host, port=20554, delay_ms=1000, connect_timeout=60, password=None):
        self.host = host
        self.port = port
    ...
    def _send_command(self, operation, ack=None):
        JVC_GREETING = 'PJ_OK'.encode()
        # if using NZ, convert password to hex and append to call
        if self.password:
            JVC_REQ = f'PJREQ_{self.password}'.encode()
        else:
            JVC_REQ = 'PJREQ'.encode()
        JVC_ACK = 'PJACK'.encode()
        result = None
    ...
    def power_on(self) -> str:
        return self._send_command(Commands.power_on.value, ack=ACKs.power_ack.value)

    def power_off(self) -> None:
        return self._send_command(Commands.power_off.value, ack=ACKs.power_ack.value)
    
    def low_latency_on(self) -> None:
        """
        NZ and up require certain params to be disabled first
        This function sets all those to be true
        """
    def command(self, command_string):
        if not hasattr(Commands, command_string):
            return False
        else:
            # TODO: make this return the commands success not if its found
            # and pass in ack
